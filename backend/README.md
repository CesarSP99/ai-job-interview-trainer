# 📄 AI Interview Coach & Smart Resume Matcher (FastAPI + Gemini + Whisper + Embeddings)

This project is a fully integrated **AI-powered career preparation platform** that combines:

- 📄 **Smart Resume Parsing & Job Matching**
- 🎤 **AI Interview Training (text + voice)**
- 🧠 **Sentiment + Emotion Analysis**
- 🤖 **Adaptive LLM-driven interview flow**
- 📊 **Explainable evaluation and feedback**
...
### 🎤 **AI Interview Training (New!)**
The user can click **“Train for this job”** and start an interview simulation.
...
│   │   ├── resume.py          # Resume parsing + job matching
│   │   ├── jobs.py            # Job CRUD & search
│   │   └── interview_train.py # Interview session API (text + audio)
│   ├── services/
│   │   ├── interview_service.py # Interview flow logic
│   │   ├── llm.py               # Gemini client, prompts
│   │   ├── audio.py             # Whisper STT + sentiment/emotion
│   │   └── extract_skills.py    # Resume skill extraction pipeline
│   ├── db/
...
## 🎤 Interview Logic Summary

1. User selects job → backend starts session  
2. Gemini generates first question  
3. User responds via **text or voice**  
4. Whisper → transcription  
5. Sentiment & emotions extracted  
6. Stored in database  
7. Gemini analyzes context → next question  
8. User clicks “Finish” → Gemini produces final evaluation  
